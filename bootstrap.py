...
supported_build_language = ["js", "cs"] #for windows
...
for parameter in sys.argv:
    ...
    elif(parameter in supported_build_language):
        args["build_language"] = parameter
...
if (args["system"].find('win') >= 0):
    ...
    if (buildSystem == 'ninja'):
        toolchainFile = os.path.join(".", "vendor/cmake/msvc/msvc.toolchain.cmake")
    ...
    #set cs/js flag for windows dll
    if (args["build_language"] == "js"):
        paramMake += " -DBUILD_JS_SDK=ON"
    elif (args["build_language"] == "cs"):
        paramMake += " -DBUILD_CS_SDK=ON"
    else:
        pass           
    ...
    toolchainFile = os.path.join(".", "vendor/cmake/ios/ios.toolchain.cmake")
    ...
    toolchainFile = os.environ["NDK_ROOT"] + "/build/cmake/android.toolchain.cmake"
    build_system = "Ninja" if (buildSystem == 'ninja') else "Unix Makefiles"
    paramMake = '-G "%s" -DANDROID_STL=c++_static -DCMAKE_BUILD_TYPE=%s -DANDROID_ABI=armeabi-v7a -DANDROID_NATIVE_API_LEVEL=19 -DCMAKE_TOOLCHAIN_FILE=%s' % (build_system, build_type, toolchainFile)
else:
    pass
